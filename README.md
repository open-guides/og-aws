...
-   **Password-based SSH:** If you are enabling password-based SSH auth by changing `sshd_config` files (e.g. toggling `PasswordAuthentication yes`) and then creating AMIs, review this [AWS forum post](https://forums.aws.amazon.com/message.jspa?messageID=467331) first.  The byline here is to make sure that you're also changing relevant cloud-init settings prior to creating the AMI.
