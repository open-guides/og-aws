...
-	❗**Enable [MFA](https://aws.amazon.com/iam/details/mfa/)** on your account.
	...
	- Instead of using long-term API keys when using the AWS APIs from the commandline it's better to use temporary keys from [Security Token Service](http://docs.aws.amazon.com/STS/latest/APIReference/Welcome.html). By applying a policy to users its also possible to require MFA authentication for any privileged API calls. Tools like [stscreds](https://github.com/uswitch/stscreds) can help this.
