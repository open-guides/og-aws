...
-	❗**Enable [MFA](https://aws.amazon.com/iam/details/mfa/)** on your account.
	...
	- Instead of using long-term API keys when using the AWS APIs from the command-line it's better to use temporary keys from [Security Token Service](http://docs.aws.amazon.com/STS/latest/APIReference/Welcome.html). By applying a policy to users it's also a good idea to require MFA authentication for any privileged API calls. Tools like [stscreds](https://github.com/uswitch/stscreds) can help this.
